...
ff_layout_read_pagelist(struct nfs_pgio_header *hdr)
{
	...
	ds = nfs4_ff_layout_prepare_ds(lseg, mirror, mirror_ds, dss_id, OP_READ);
	if (IS_ERR(ds)) {
...
ff_layout_write_pagelist(struct nfs_pgio_header *hdr, int sync)
{
	...
	dss_id = nfs4_ff_layout_calc_dss_id(
		...
		offset);
	mirror_ds = ff_layout_get_mirror_ds(lseg->pls_layout, mirror, dss_id);
	ds = nfs4_ff_layout_prepare_ds(lseg, mirror, mirror_ds, dss_id, OP_WRITE);
	if (IS_ERR(ds)) {
		ds_fatal_error = nfs_error_is_fatal(PTR_ERR(ds));
		goto out_failed;
	}
...
static int ff_layout_initiate_commit(struct nfs_commit_data *data, int how)
{
	...
	mirror_ds = ff_layout_get_mirror_ds(lseg->pls_layout, mirror, dss_id);
	ds = nfs4_ff_layout_prepare_ds(lseg, mirror, mirror_ds, dss_id, OP_COMMIT);
	if (IS_ERR(ds))
		goto out_err;
