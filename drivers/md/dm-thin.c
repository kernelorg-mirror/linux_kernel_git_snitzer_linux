...
static dm_block_t get_first_alloc_group_aligned_block(dm_block_t virt_block,
						      struct pool *pool)
{
...
static int __alloc_data_blocks(struct thin_c *tc, dm_block_t bio_virt_block,
			       dm_block_t *result)
{
	int r;
	...
	first_virt_block = get_first_alloc_group_aligned_block(bio_virt_block, pool);
	last_virt_block = first_virt_block + pool->pf.blocks_per_allocation - 1;

	/*
	 * Important to allocate data_blocks in-order, to have contiguous
	 * allocation from pool, e.g.:
	 * <bio-less blocks> <bio-full @bio_virt_block> <more bio-less blocks>
	 */
	for (virt_block = first_virt_block; virt_block <= last_virt_block; virt_block++) {
		struct dm_bio_prison_cell *cell = NULL;

		/*
		 * Must lock bio-less virt_block(s) in a cell for this thin dev;
		 * the bio-full @bio_virt_block is already locked in a cell.
		 */
...
/*
 * Allocate one or more data blocks.  If @bio_virt_block is not NULL the caller
 * allows multiple blocks to be allocated (governed by blocks_per_allocation).
 */
static int alloc_data_blocks(struct thin_c *tc, dm_block_t *bio_virt_block,
			     dm_block_t *result)
{
	int r;
	dm_block_t free_blocks;
	...
	if (bio_virt_block && pool->pf.blocks_per_allocation > 1 &&
	    free_blocks >= pool->pf.blocks_per_allocation)
		return __alloc_data_blocks(tc, *bio_virt_block, result);
...
{
	return alloc_data_blocks(tc, NULL, result);
}
			    ...
			    struct dm_bio_prison_cell *cell)
{
	...
	r = alloc_data_blocks(tc, &block, &data_block);
	switch (r) {
	case 0:
		if (tc->origin_dev)
