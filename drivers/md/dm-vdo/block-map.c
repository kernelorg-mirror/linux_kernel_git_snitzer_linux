...
static int reset_page_info(struct page_info *info)
{
	...
	result = ASSERT(!vdo_waitq_has_waiters(&info->waiting), "VDO Page must not have waiters");
	if (result != UDS_SUCCESS)
		return result;
...
static void set_persistent_error(struct vdo_page_cache *cache, const char *context, int result)
{
	...
	vdo_waitq_notify_all_waiters(&cache->free_waiters, complete_waiter_with_error, &result);
...
int vdo_invalidate_page_cache(struct vdo_page_cache *cache)
{
	...
	return vdo_hash_map_create(HASH_MAP_TYPE_INT, cache->page_count, &cache->page_map);
}
...
static void finish_block_map_allocation(struct vdo_completion *completion)
{
	...
	vdo_waitq_notify_all_waiters(&tree_lock->waiters, continue_allocation_for_waiter, &pbn);
	if (tree_lock->height == 0) {
		finish_lookup(data_vio, VDO_SUCCESS);
		return;
	}
