...
/**
 * submit_vio() - Submits a vio's bio to the underlying block device. May block if the device
...
static struct bio *get_bio_list(struct vio *vio)
{
	...
	vdo_hash_map_remove(bio_queue_data->map,
...
{
	vdo_hash_map_remove(bio_map, &prev_vio->bios_merged.tail->bi_iter.bi_sector);
...
static int merge_to_next_head(struct vdo_hash_map *bio_map, struct vio *vio, struct vio *next_vio)
{
	 ...
	 * existing completion.
	 */
	vdo_hash_map_remove(bio_map, &next_vio->bios_merged.head->bi_iter.bi_sector);
			   ...
			   unsigned int operation, char *data)
{
	...
	vdo_set_completion_callback(completion, submit_vio, get_vio_bio_zone_thread_id(vio));
