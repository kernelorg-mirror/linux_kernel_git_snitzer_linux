				   ...
				   struct vdo_wait_queue *waitq)
{
	...
	vdo_waitq_notify_all_waiters(waitq, NULL, &result);
}
...
{
	vdo_waitq_notify_all_waiters(&slab->dirty_blocks, launch_reference_block_write, slab);
	check_if_slab_drained(slab);
}
	...
	if (length < journal->flushing_deadline) {
		...
		write_count =
			max_t(block_count_t, (write_count /
					      (journal->flushing_deadline - length + 1)), 1);
	}
...
static void notify_block_allocator_of_read_only_mode(void *listener, struct vdo_completion *parent)
{
	...
	while (iterator.next != NULL) {
		...
		vdo_waitq_notify_all_waiters(&slab->journal.entry_waiters, abort_waiter, &slab->journal);
		check_if_slab_drained(slab);
	}
